...
const getStringFromCharCodeRange = (min, max) => ' '.repeat(max - min + 1).split('')
  .map((v, i) => String.fromCharCode(min + i))
  .join('')

const CHAR_LIST_STRING = [
  getStringFromCharCodeRange(32, 47),
  getStringFromCharCodeRange(48, 63),
  getStringFromCharCodeRange(64, 79),
  getStringFromCharCodeRange(80, 95),
  getStringFromCharCodeRange(96, 111),
  getStringFromCharCodeRange(112, 126),
  ... RAW_CHAR_STRING_LIST
    .map((v) => v.replace(/[\x00-\x7F]/g, ''))
].join('\n')

require('fs').writeFileSync('char-list.txt', CHAR_LIST_STRING)
